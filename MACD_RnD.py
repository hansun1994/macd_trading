...
while datetime.datetime.now() >= open_time:
    ...
    if time_now >= trade_start:
        for ticker in tickers:
            if macd_table.ix[-1, ticker] < 0 & macd_table.ix[-2,ticker] > 0:
                signal_now = 'BUY'
                
            elif macd_table.ix[-1, ticker] > 0 & macd_table.ix[-2,ticker] < 0:
                signal_now = 'SELL'
